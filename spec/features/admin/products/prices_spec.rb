  ...
  context "#product without variants" do
    ...
    context "#using an explicit price book" do
      ...
      it "lists the prices in text fields", js: true do
        expect(page.all('table tbody tr td input[type=text]').count).to eq product.variants_including_master.size
      end
    ...
    context "#using a factored price book" do
      ...
      it "lists the prices as read-only", js: true do
        expect(page.all('table tbody tr td input[type=text]').count).to eq 0
        expect(find('table tbody tr td:nth-child(3)')).to have_text(factored_price_book.prices.find_by_variant_id(product.master).amount)
      end
    end
    ...
    context "#product with multiple variants" do
      ...
      it "lists each variant in the prices tabale" do
        expect(page.all('table tbody tr').count).to eq product.variants_including_master.size
      end
